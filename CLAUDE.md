...
**Sync** — batch version check → **slim outdated gate table** (name/version/update/pin/repo) → confirm (the user can bail before any release fetch or download is spent). Only after opt-in does it prompt: per package, fetch release and re-select each stored asset by hint (prompting only when ambiguous; a skipped package drops out) → parallel download + overlay-extract → rebuild shims (conflict check, preserving prior shim names where the binary path is unchanged) and reinstall fonts (conflict check, preserving prior names) → write manifest → per-package `bin`/`font` child lines and a `✓ updated N` completion line. The gate table is built from the version check alone, so it precedes all prompting. No "here's what happened" summary table — the gate (preview) plus the per-package child lines and the `✓` count are enough to show the work completed.
